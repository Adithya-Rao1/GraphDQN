...
from webapp.backend.rl_bridge.dqn_runner import TrainingCancelled
from webapp.backend.rl_bridge.reward_config import blend_reward, normalize_rating
...
def finetune_dqn(
    ...
    init_mol: str,
    seed: int,
    num_extra_episodes: int,
    alpha: float,
    scored_candidates: List[dict],  # [{"smiles": str, "user_rating": float 0-100}, ...]
    ...
    max_steps = max_steps or hyp.max_steps

    agent = DKDQNAgent(output_dim=1, device=device)
    state_dict = torch.load(parent_checkpoint_path, map_location=device)
    ...
    rating_lookup = {
        _canonical(c["smiles"]): normalize_rating(c["user_rating"]) for c in scored_candidates
    }

    def _blended_reward(smiles: str, computed_reward: float) -> float:
        normalized = rating_lookup.get(_canonical(smiles))
        if normalized is None:
            return computed_reward
        return blend_reward(computed_reward, normalized, alpha)

    for candidate_smiles, normalized_rating in (
        (c["smiles"], rating_lookup[_canonical(c["smiles"])]) for c in scored_candidates
    ):
        env = MultiObjectiveRewardEnv(
            discount_factor=hyp.discount_factor, device=device, init_mol=candidate_smiles,
            max_steps=max_steps, target_seq=target_seq, off_target_seq=off_target_seq,
            admet_model=admet_model, binding_model=binding_model, sa_model=sa_model,
            reward_config=reward_config,
        )
        env.initialize()
        next_valid_actions = list(env.get_valid_actions())
        computed = compute_reward(
            smiles=_canonical(candidate_smiles), target_seq=target_seq, device=device,
            off_target_seq=off_target_seq, admet_model=admet_model, binding_model=binding_model,
            sa_model=sa_model, **reward_config.to_compute_reward_kwargs(),
        )["reward"]
        blended = blend_reward(computed, normalized_rating, alpha)
        agent.replay_buffer.add(
            obs_t=candidate_smiles, action=0, reward=blended, obs_tp1=next_valid_actions, done=1.0,
        )

    checkpoint_dir = os.path.join(checkpoint_root, run_id)
    batch_losses = []
    episode_rewards = []
    eps_start, eps_end = 0.5, 0.1  # warm start
    tau_start, tau_end = 1.0, 0.1
    start_time = time.time()

    for episode in range(num_extra_episodes):
        if cancel_event is not None and cancel_event.is_set():
            raise TrainingCancelled(f"Fine-tuning cancelled at episode {episode}/{num_extra_episodes}")

        progress = episode / num_extra_episodes
        ...
        environment = MultiObjectiveRewardEnv(
            discount_factor=hyp.discount_factor, device=device, init_mol=init_mol, max_steps=max_steps,
            target_seq=target_seq, off_target_seq=off_target_seq, admet_model=admet_model,
            binding_model=binding_model, sa_model=sa_model, reward_config=reward_config,
        )
        ...
        for _step in range(max_steps):
            ...
            reward = _blended_reward(action_obs, reward)
        ...
        if progress_cb is not None:
            progress_cb(episode + 1, num_extra_episodes, final_reward)
    ...
    return {
        ...
        "seed": seed,
        "num_episodes": num_extra_episodes,
